...
export default function SuperAdmin({
    ...
    const barChartData = tongPerUnit && tongPerUnit.length > 0 ? {
        ...
        datasets: [
            {
                label: 'Total Tong',
                data: tongPerUnit.map(item => item.total_tong || item.total || 0),
    ...
    const lineChartData = trenPengangkutan && trenPengangkutan.length > 0 ? {
        ...
        datasets: [
            {
                label: 'Pengangkutan',
                data: trenPengangkutan.map(item => item.jumlah || item.count || 0),
                borderColor: '#16a34a',
        ...
        <AppLayout header={header}>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                    ...
                    />
                    <StatCard
                        title="Aduan Menunggu"
                        value={totalAduanMenunggu || 0}
                        color="yellow"
                        icon={
                            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
